    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
                ...
                <form class="needs-validation" action="{{route('session.update',[$event,$session])}}"
                    ...
                    <div class="row">
                        <div class="col-12 col-lg-4 mb-3">
                            ...
                            <select class="form-control" id="selectType" name="type">
                                ...
                                'type',$session->type)=='workshop' ?'selected':''}}>Workshop
                                </option>
                            </select>
                        </div>
                    </div>
                            ...
                            <select class="form-control" id="selectRoom" name="room">
                                @foreach($event->channels as $channel)
                                    @foreach($channel->rooms as $room)
                                        <option
                                            value="{{$room->id}}" {{$session->room_id == $room->id?'selected' :''}}>{{$room->name}}</option>
                                    @endforeach
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-12 col-lg-6 mb-3">
                            ...
                            <input type="text"
                                   ...
                                   value="{{old(
                                'start',date('Y-m-d H:i',strtotime($session->start)))}}">
                            @error('start')
                            <div class="invalid-feedback">
                                {{$message}}
                            </div>
                        ...
                        <div class="col-12 col-lg-6 mb-3">
                            ...
                            <input type="text"
                                   ...
                                   placeholder="yyyy-mm-dd HH:MM"
                                   value="{{old(
                                'end',date('Y-m-d H:i',strtotime($session->end)))}}">
                            @error('end')
                            <div class="invalid-feedback">
                                {{$message}}
                            </div>
